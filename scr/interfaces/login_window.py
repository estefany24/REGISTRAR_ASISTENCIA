...
class LoginWindow:
    def __init__(self, master):
        ...
        # Etiqueta de Usuario
        tk.Label(self.master, text="Usuario:").grid(row=0, column=0, padx=10, pady=10)
        self.entry_usuario = tk.Entry(self.master)
        self.entry_usuario.grid(row=0, column=1, padx=10, pady=10)

        # Etiqueta de Contraseña
        tk.Label(self.master, text="Contraseña:").grid(row=1, column=0, padx=10, pady=10)
        self.entry_contrasena = tk.Entry(self.master, show="*")
        self.entry_contrasena.grid(row=1, column=1, padx=10, pady=10)

        # Botón de Iniciar Sesión
        tk.Button(self.master, text="Iniciar Sesión", command=self.iniciar_sesion).grid(row=2, column=0, columnspan=2, padx=10, pady=10)
    ...
    def iniciar_sesion(self):
        ...
        # Aquí puedes agregar la lógica de verificación del usuario
        if usuario and contrasena == comparar_contraseña:  # Ejemplo simple
            messagebox.showinfo("Éxito", "Inicio de sesión exitoso")
            self.master.destroy()  # Cierra la ventana de inicio de sesión
            self.abrir_ventana_admin()  # Abre la ventana principal
        else:
            messagebox.showerror("Error", "Usuario o contraseña incorrectos")

    def continuar_como_usuario(self):
        self.master.destroy()  # Cierra la ventana de inicio de sesión
        self.abrir_ventana_general()  # Abre la ventana principal

    def abrir_ventana_general(self):
        import sys
        import os
        sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__))))
        from registra_asistencia import RegistrarAsistencia  # Asegúrate de importar la clase MainWindow
        ventana_principal = tk.Tk()
        app = RegistrarAsistencia(ventana_principal)
        ventana_principal.mainloop()
    ...
    def abrir_ventana_admin(self):
        ...
        import os
        sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__))))
        from main_window import MainWindow  # Asegúrate de importar la clase MainWindow
        ventana_principal = tk.Tk()
        app = MainWindow(ventana_principal)
        ventana_principal.mainloop()
'''
